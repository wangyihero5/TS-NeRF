...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid',stage="first_stage"):
        super().__init__()
    ...
    def forward(self, x, d, **kwargs):
        ...
        rgbs = self.rgb_net(torch.cat([d, h], 1))  #  h = self.xyz_encoder(x)
        # 不进入
    ...
    def mark_invisible_cells(self, K, poses, img_wh, chunk=64**3):
        ...
        for c in range(self.cascades):
            ...
            for i in range(0, len(indices), chunk):
                ...
                # xyzs = (coords[i:i + chunk] / (self.grid_size - 1) * 2 - 1).to(torch.half)
                # K = K.to(torch.half)
                ...
                # self.count_grid[c, indices[i:i+chunk]] =  count = (covered_by_cam.sum(0)/N_cams).to(torch.half)
                ...
                # self.density_grid[c, indices[i:i+chunk]] = (torch.where(valid_mask, 0., -1.)).to(torch.half)
