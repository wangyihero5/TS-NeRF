...
class CLIPLoss(torch.nn.Module):
    ...
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., patch_loss_type='mae', direction_loss_type='cosine',
                 clip_model='ViT-L/14'):  # ViT-L/14  ViT-B/32
        super(CLIPLoss, self).__init__()
        if not CLIPLoss._flag:
            CLIPLoss._flag = True
        ...
        self.enable_ArtBench_search = False  # Whether to enable ArtBench searcher

    def set_ArtBench_search(self, enable=False):
        self.enable_ArtBench_search = enable
    ...
    def encode_images_with_cnn(self, images: torch.Tensor) -> torch.Tensor:
        images = self.preprocess_cnn(images).to(self.device)  # # Un-normalize from [-1.0, 1.0] (GAN output) to [0, 1].
    ...
    def compute_text_direction(self, source_class: str, target_class: str) -> torch.Tensor:
        source_features = self.get_text_features(source_class)  # the encoder of source text
        target_features = self.get_text_features(target_class)  # the encoder of target text
        if self.enable_ArtBench_search and self.target_features == None:
            target_features = nearest_neighbor_searcher.get_nearest_neighbor_vector(target_features)
            target_features = target_features.mean(1)
