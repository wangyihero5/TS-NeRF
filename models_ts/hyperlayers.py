...
# hero instant's style model
class HyperFC_instant(nn.Module):
    '''Builds a hypernetwork that predicts a fully connected neural network.
    '''

    def __init__(self,
                 in_ch_pos,  # MLP input dim (3D points)   :16
                 in_ch_view,  # MLP input dim (view direction)  :16
                 out_ch,  # MLP output dim (rgb)  :3
                 hyper_in_ch=512,  # Hyper input dim (embedding)
                 hyper_num_hidden_layers=1,
                 hyper_hidden_ch=64,  # Hyper output dim (hidden)
                 # hidden_ch=128, # MLP layer dim
                 hidden_ch=16,  # MLP layer dim
                 num_hidden_layers=6,  # Total number of MLP layers
                 ):
        super().__init__()
        ...
        self.layers = nn.ModuleList()
        self.layers.append(PreconfHyperLinear(in_ch=hidden_ch, out_ch=hidden_ch * 4))  # base_remap
        self.layers.append(PreconfHyperLayer(in_ch=hidden_ch * 4 + in_ch_view, out_ch=hidden_ch * 2))  # rgb_layer0
        self.layers.append(PreconfHyperLayer(in_ch=hidden_ch * 2, out_ch=hidden_ch * 2))  # rgb_layer1
        self.layers.append(PreconfHyperLayer(in_ch=hidden_ch * 2, out_ch=hidden_ch))  # rgb_layer2
        self.layers.append(PreconfHyperLinear(in_ch=hidden_ch, out_ch=out_ch))  # rgb_layer3
